...
export default async function ArticlePage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const paragraphs = article.content.split('\n\n').map((block, i) => {
    if (block.startsWith('## ')) {
      return <h2 key={i} style={{ fontFamily: 'var(--font-playfair)', color: '#111116', fontSize: '1.25rem', fontWeight: 300, marginTop: '2rem', marginBottom: '0.75rem' }}>{block.replace('## ', '')}</h2>
    }
    if (block.startsWith('### ')) {
      return <h3 key={i} style={{ fontFamily: 'var(--font-playfair)', color: '#111116', fontSize: '1.05rem', fontWeight: 400, marginTop: '1.5rem', marginBottom: '0.5rem' }}>{block.replace('### ', '')}</h3>
    }
    if (block.includes('\n- ')) {
    ...
    <>
      ...
      <main style={{ background: '#f4f1ed', minHeight: '100vh' }}>
        <div className="max-w-3xl mx-auto px-6 pt-36 pb-24">
          ...
          <h1 className="font-light mb-4 leading-tight" style={{ fontFamily: 'var(--font-playfair)', color: '#111116', fontSize: 'clamp(1.5rem, 3vw, 2rem)' }}>
            {article.title}
          </h1>
          ...
          <div style={{ borderTop: '1px solid rgba(138,21,53,0.15)', paddingTop: '2rem' }}>
            <p className="text-sm mb-8 leading-relaxed" style={{ color: '#111116', fontStyle: 'italic', fontFamily: 'var(--font-playfair)' }}>
              {article.excerpt}
            </p>
          ...
          <div className="mt-16 p-7 rounded-sm" style={{ background: '#111116' }}>
            <p className="text-sm text-white mb-1" style={{ fontFamily: 'var(--font-playfair)' }}>Une question sur votre situation ?</p>
